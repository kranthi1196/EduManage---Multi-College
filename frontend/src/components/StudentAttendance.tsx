...
const StudentAttendanceView: React.FC<StudentAttendanceProps> = ({ 
  ...
  const fetchStudents = useCallback(async () => {
    ...
    // Require active filtering (Year, Semester, RollNo) to show data
    const areFiltersActive = selectedYear !== 'all' || 
  ...
  const renderTableBody = () => {
    ...
    const isSearchActive = nameQuery || admissionNoQuery || selectedYear !== 'all' || selectedSemester !== 'all' || selectedRollNo !== 'all';
    if (isSearchActive) {
  ...
  const commonSelectClass = "bg-slate-800 border border-slate-600 text-white text-sm rounded-md focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 disabled:bg-slate-700 disabled:text-slate-500";
