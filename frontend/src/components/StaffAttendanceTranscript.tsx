...
import { StaffDetailsType } from '../types/index';
import PrintLayout from './PrintLayout';

interface StaffAttendanceTranscriptProps {
  staff: StaffDetailsType;
  academicYear: string;
}

const StaffAttendanceTranscript: React.FC<StaffAttendanceTranscriptProps> = ({ staff, academicYear }) => {
      ...
      <div id="transcript-container" className="w-full bg-white text-black font-sans">
        {attendanceChunks.length > 0 ? attendanceChunks.map((chunk, index) => (
            ...
            >
                  ...
                  <table className="w-full my-2 text-xs border-collapse border border-black">
                     <tbody>
                        <tr className="border-b border-black">
                            <td className="font-bold p-1 w-[15%] border-r border-black">Staff Name:</td>
                            <td className="p-1 w-[35%] border-r border-black">{staff.staffName.toUpperCase()}</td>
                            <td className="font-bold p-1 w-[20%] border-r border-black">Staff ID:</td>
                            <td className="p-1 w-[30%]">{staff.staffId}</td>
                        </tr>
                        <tr className="border-b-0">
                            <td className="font-bold p-1 border-r border-black">Department:</td>
                            <td className="p-1 border-r border-black">{staff.programCode}</td>
             ...
             <PrintLayout person={staff} currentPage={1} totalPages={1} isLastPage={true} reportTitle="Staff Attendance Report">
...
export default StaffAttendanceTranscript;