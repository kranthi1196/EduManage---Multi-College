...
import { getDashboardData, getFilteredStudentDetails, getStudentDetails, getSubjects, getFilteredFacultyDetails, getFilteredStaffDetails } from '../services/api';
// FIX: Import ACADEMIC_YEARS to resolve module member error.
import { COLLEGE_NAMES, LATEST_ATTENDANCE_DATE, DEPARTMENTS, ACADEMIC_YEARS } from '../constants';
...
const Dashboard: React.FC<DashboardProps> = ({ user, onToggleSidebar, selectedDate, setSelectedDate }) => {
    const [loading, setLoading] = useState(true);
    ...
    useEffect(() => {
        if (selectedFacultyId) {
          setSelectedDepartment('all'); setSelectedYear('all'); setSelectedRollNo('all'); setSelectedStaffId('');
      ...
      useEffect(() => {
          if (selectedStaffId) {
            setSelectedDepartment('all'); setSelectedYear('all'); setSelectedRollNo('all'); setSelectedFacultyId('');
            setSelectedSemester('all'); setSelectedSubject('All Subjects');
          }
      }, [selectedStaffId]);
      ...
      }, [selectedDepartment, selectedYear, selectedRollNo, selectedSemester, selectedSubject]);
    ...
    const handleDownload = async () => {
        ...
        try {
            if (debouncedFacultyId) {
                const data = await getFilteredFacultyDetails(debouncedFacultyId, selectedDate, selectedDate);
                ...
                downloadCsv([headers.join(','), ...rows], `faculty_${debouncedFacultyId}_data.csv`);
    
            } else if (debouncedStaffId) {
                const data = await getFilteredStaffDetails(debouncedStaffId, selectedDate, selectedDate);
                if (data.length === 0) { alert('No data found.'); return; }
                const headers = Object.keys(data[0]);
                const rows = data.map(row => headers.map(header => JSON.stringify(row[header as keyof typeof row])).join(','));
                downloadCsv([headers.join(','), ...rows], `staff_${debouncedStaffId}_data.csv`);
    
            } else {
                const data = await getFilteredStudentDetails(selectedCollege, selectedYear, selectedDepartment, selectedRollNo, selectedDate, selectedDate, selectedSemester, selectedSubject);
                if (data.length === 0) { alert('No data to download.'); return; }
                const headers = Object.keys(data[0]);
                const rows = data.map(row => headers.map(header => JSON.stringify(row[header as keyof typeof row])).join(','));
                downloadCsv([headers.join(','), ...rows], 'student_data.csv');
            }
        } catch (error) { console.error("CSV Download failed:", error); alert('Download failed.');
    ...
    const isIndividualFilterActive = !!selectedFacultyId || !!selectedStaffId;
    const isStudentFilterActive = selectedDepartment !== 'all' || selectedYear !== 'all' || selectedRollNo !== 'all' || selectedSemester !== 'all' || selectedSubject !== 'All Subjects';
                ...
                {isFilterOpen && (
                    <div className="pt-4 border-t border-slate-700 space-y-4">
                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 mb-4">
                            <div><label className="block text-xs font-medium text-slate-400 mb-1">College</label><select name="college" value={selectedCollege} onChange={(e) => setSelectedCollege(e.target.value as College)} className={commonSelectClass} disabled={user.role !== Role.CHAIRMAN}>{Object.entries(COLLEGE_NAMES).map(([key, name]) => (<option key={key} value={key}>{name}</option>))}</select></div>
                            <div><label className="block text-xs font-medium text-slate-400 mb-1">Department</label><select name="department" value={selectedDepartment} onChange={(e) => setSelectedDepartment(e.target.value)} className={commonSelectClass} disabled={isIndividualFilterActive}><option value="all">All</option>{DEPARTMENTS.map(dept => (<option key={dept} value={dept}>{dept}</option>))}</select></div>
