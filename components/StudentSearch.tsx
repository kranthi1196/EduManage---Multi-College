...
import { User, Student, StudentDetailsType, College, Role } from '../types';
import { searchStudents, getStudentDetails } from '../services/api';
// FIX: Imported DownloadIcon.
import { SearchIcon, ChevronDownIcon, ChevronUpIcon, ChevronUpDownIcon, MenuIcon, XMarkIcon, DownloadIcon } from './icons';
import StudentDetails from './StudentDetails';
import { COLLEGE_NAMES, DEPARTMENTS, ACADEMIC_YEARS } from '../constants';

// FIX: Added props for download functionality and shared filters.
...
const StudentSearch: React.FC<StudentSearchProps> = ({ 
  ...
  const fetchStudents = useCallback(async () => {
    ...
    try {
      const studentResults = await searchStudents(
        ...
        selectedCollege, selectedDepartment, selectedYear, selectedRollNo, selectedSemester
      );
  ...
  }, [debouncedNameQuery, debouncedAdmissionNoQuery, selectedCollege, selectedDepartment, selectedYear, selectedSemester, selectedRollNo, user.role]);
  ...
  const renderTableBody = () => {
    if (loading) return <tr><td colSpan={4} className="text-center py-8 text-slate-400">Loading...</td></tr>;
    if (error) return <tr><td colSpan={4} className="text-center py-8 text-red-500">{error}</td></tr>;
    if (sortedResults.length > 0) return sortedResults.map((student) => (
        <tr key={student.admissionNumber} onClick={() => handleRowClick(student.admissionNumber)} onKeyDown={(e) => e.key === 'Enter' && handleRowClick(student.admissionNumber)} className="hover:bg-slate-800 cursor-pointer focus:outline-none focus:bg-slate-700" tabIndex={0} role="button">
    ...
    const isSearchActive = nameQuery || admissionNoQuery || selectedCollege !== College.ALL || selectedDepartment !== 'all' || selectedYear !== 'all' || selectedSemester !== 'all' || selectedRollNo !== 'all';
    if (isSearchActive) return <tr><td colSpan={4} className="text-center py-8 text-slate-400">No students found for the current criteria.</td></tr>;
    
    return <tr><td colSpan={4} className="text-center py-8 text-slate-400">Apply filters or enter a search query to begin.</td></tr>;
  };
    ...
    <main className="flex-1 p-4 md:p-6 lg:p-8 bg-transparent overflow-y-auto">
      <div className="flex flex-wrap justify-between items-center gap-4 mb-6">
          <div className="flex items-center gap-4">
              ...
              <h2 className="text-2xl font-bold text-white">Student Marks Search</h2>
          </div>
          <button onClick={() => onDownload('studentSearch')} disabled={isDownloading} className="flex items-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white font-semibold py-2 px-4 rounded-lg transition disabled:bg-slate-600">
      ...
      <div className="mb-6 p-6 bg-slate-900 rounded-xl shadow-lg border border-slate-800 space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4">
          ...
          <div>
            <label className="block text-sm font-medium text-slate-300 mb-1">Admission Year</label>
      ...
      <div className="bg-slate-900 rounded-xl shadow-lg border border-slate-800 overflow-hidden">
        <div className="overflow-x-auto"><table className="min-w-full divide-y divide-slate-700"><thead className="bg-slate-800"><tr>
            ...
            </tr></thead><tbody className="bg-slate-900 divide-y divide-slate-700">{renderTableBody()}</tbody></table></div>
      </div>
      {(selectedStudent || modalLoading) && (modalLoading ? <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50"><div className="text-white bg-slate-800 px-4 py-2 rounded-lg shadow-lg">Loading details...</div></div> : selectedStudent && <StudentDetails student={selectedStudent} onClose={() => setSelectedStudent(null)} />)}
    </main>
  );
};
