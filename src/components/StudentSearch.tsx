...
const StudentSearch: React.FC<StudentSearchProps> = ({ 
  ...
  const fetchStudents = useCallback(async () => {
    ...
    try {
      ...
      // FIX: Pass selectedPlacement as the 7th argument to match the function signature in src/services/api.ts.
  ...
  }, [debouncedNameQuery, debouncedAdmissionNoQuery, selectedCollege, selectedDepartment, selectedYear, selectedSemester, selectedRollNo, selectedPlacement, user.role]);
  ...
  const renderTableBody = () => {
    ...
    const isSearchActive = nameQuery || admissionNoQuery || selectedCollege !== College.ALL || selectedDepartment !== '' || selectedYear !== '' || selectedSemester !== '' || selectedRollNo !== '' || selectedPlacement !== '';
    if (isSearchActive) return <tr><td colSpan={colSpan} className="text-center py-8 text-slate-400">No students found for the current criteria.</td></tr>;
    
    return <tr><td colSpan={colSpan} className="text-center py-8 text-slate-400">Apply filters or enter a search query to begin.</td></tr>;
  };

  const commonSelectClass = "bg-slate-800 border border-slate-600 text-white text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5";
      ...
      <div className="mb-6 p-6 bg-slate-900 rounded-xl shadow-lg border border-slate-800 space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-4">
          ...
          <div>
            ...
            <select value={selectedPlacement} onChange={(e) => setSelectedPlacement(e.target.value)} className={commonSelectClass}>
                <option value="">All</option>
