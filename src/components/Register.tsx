...
const Register: React.FC<RegisterProps> = ({ onRegister, switchToLogin }) => {
  ...
  const [college, setCollege] = useState<College>(College.BRIG);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (role === Role.STUDENT) {
        ...
        const studentIdRegex = new RegExp(`^${expectedPrefix}(${DEPARTMENTS.join('|')})(\\d{4})(\\d{2})$`);
        const match = username.toUpperCase().match(studentIdRegex);
        if (!match) {
            setError('Invalid Admission Number format. Expected: [Prefix(1)][Dept][Year][Roll(2)]');
            return;
        }
        const rollNumber = parseInt(match[3], 10);
        if (rollNumber < 1 || rollNumber > 60) {
            setError('Invalid Roll Number. Must be between 01 and 60.');
            return;
        }
        ...
        if (role === Role.FACULTY || role === Role.HOD) {
            ...
            if (!facultyIdRegex.test(username.toUpperCase())) {
                setError('Invalid Faculty/HOD ID. Expected format: [CollegePrefix][DeptCode][DDMMYYYY]-[SerialNo]');
                return;
            }
        } else if (role === Role.STAFF) {
            const staffIdRegex = new RegExp(`^${expectedPrefix}STF\\d{8}-\\d{3}$`);
            if (!staffIdRegex.test(username.toUpperCase())) {
                setError('Invalid Staff ID. Expected format: [CollegePrefix]STF[DDMMYYYY]-[SerialNo]');
                return;
            }
        }
    }
  ...
  const getUsernameHint = () => {
      if (role === Role.STUDENT) {
          return 'Format: [Prefix(1)][Dept][Year][Roll(2)] e.g., KCSE202001';
      }
      if (role === Role.FACULTY || role === Role.HOD) {
          return 'Format: [Prefix(1)][Dept][DDMMYYYY]-[Serial] e.g., KCSE15042020-001';
      }
      if (role === Role.STAFF) {
          return 'Format: [Prefix(1)]STF[DDMMYYYY]-[Serial] e.g., KSTF15042020-001';
      }
      return null;
  };
            ...
            <div>
              ...
              <select value={college} onChange={e => setCollege(e.target.value as College)} className={inputClass('college')} onFocus={() => setFocused('college')} onBlur={() => setFocused(null)}>
                {Object.entries(COLLEGE_NAMES)
                  ...
                  .map(([key, name]) => (
                    <option key={key} value={key}>{name as React.ReactNode}</option>
                  ))}
              </select>
            </div>
          )}
