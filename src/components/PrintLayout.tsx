...
const PrintLayout: React.FC<PrintLayoutProps> = ({ student, children, currentPage, totalPages, isLastPage, reportTitle }) => {
  return (
    <div className={`relative ${currentPage > 1 ? 'break-before-page' : ''}`}>
      ...
      <div className="absolute inset-0 flex items-center justify-center z-0 opacity-[0.05]">
        <CollegeLogo college={student.collegeCode} className="h-96 w-96" background="transparent" />
      </div>
          ...
          {currentPage === 1 && (
            <header className="flex justify-between items-center border-b-2 border-black pb-2 mb-2 flex-shrink-0">
              <div className="w-24 flex-shrink-0">
                <CollegeLogo college={student.collegeCode} className="h-16 w-auto" />
              </div>
              <div className="text-center">
                <h1 className="text-xl font-bold uppercase">{COLLEGE_NAMES[student.collegeCode]}</h1>
